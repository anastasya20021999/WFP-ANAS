    ...
    <div class="slider_area ">
        ...
        <div class="single_slider d-flex align-items-center justify-content-center slider_bg_1">    
            <div class="container">
                ...
                <div style = "margin : -90px 50px 40px;  text-align:center;">
				<h1 style="color:white;"> 
					Tambah Saldo Baru
				</h1>
				@if(count($errors->all()))
				@foreach($errors->all() as $error)
					<div style="background-color: red; font-weight: bold;color: white;">Sorry!! {{$error}}</div>
				@endforeach
				@endif 
                </div>
                <div style = color:white>
                    <div class="row align-items justify-content-center">
					<form method="post" action="{{url('saldos')}}">
				<!--ini kategoris method post kategoris.store liat di php artisan route:list
					liat dia ke fungsi mana disana-->
					<input type="hidden" name="_token" value="{{ csrf_token() }}"/>
					<input type="hidden" name="user" value= "{{Auth::user()->id}}"/>
					<!--ini buat password manggilnya name harus _token diiat di klik kanan page resources-->
				<h3 style = "color: white">
				<table>
				
				<tr>
					<td>Nama: </td>
					<td><input type="text" name="nama_saldo"/></td>
				</tr>
				<tr>
					<td>Nominal: </td>
					<td><input type="number" name="nominal" min="1" max="1000000000"></td>
				</tr>
